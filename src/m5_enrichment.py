...
def contextual_prepend(text: str, document_title: str = "") -> str:
    ...
    if OPENAI_API_KEY:
        ...
        return f"{context}\n\n{text}"
    # Fallback: prepend tên document nếu có
    if document_title:
...
def extract_metadata(text: str) -> dict:
    ...
    if OPENAI_API_KEY:
        ...
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": 'Trích xuất metadata từ đoạn văn. Chỉ trả về JSON hợp lệ, không giải thích thêm: {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}'},
                {"role": "user", "content": text},
            ],
            max_tokens=150,
        )
        try:
            ...
            return {"topic": "", "entities": [], "category": "other", "language": "vi"}
    return {}
